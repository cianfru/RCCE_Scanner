...
function CellContent({ colLabel, row, index, isMobile, backtestSymbols, favorites, onToggleFavorite, priceFlash }) {
  ...
  switch (colLabel) {
    ...
    case "SYMBOL": {
      ...
      // Anomaly dot shown separately so both are visible
      ...
      return (
        <td style={{ padding: cellPad, fontFamily: T.mono, fontWeight: 700, color: T.text1, fontSize: m(isMobile ? T.textMd : T.textLg, isMobile), letterSpacing: "0.02em", whiteSpace: "nowrap" }}>
          ...
          {hasAnomaly && (
            <span
              title="Anomaly — unusual activity detected, priority scanning"
              style={{
          ...
          {/* Scan tier dot (yellow=fav, cyan=active, gray=cold) */}
          <span
            title={tierLabel}
            style={{
              display: "inline-block",
