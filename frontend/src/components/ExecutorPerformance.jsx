...
export default function ExecutorPerformance({ performance: p, mode }) {
  ...
  return <section className="executor-performance">
    ...
    <div className="executor-performance-grid">
      <Metric label="Recorded realized P&L" value={money(p.realized_pnl_usd)} amount={p.realized_pnl_usd} note={`${p.closed_trades} closures · ${p.flagged_closed_trades || 0} flagged for review`} />
      <Metric label={p.priced_positions === p.open_positions ? 'Unrealized P&L' : 'Unrealized P&L · partial'} value={money(p.unrealized_pnl_usd)} amount={p.unrealized_pnl_usd} note={`${p.priced_positions} of ${p.open_positions} valued · ${p.open_profitable_count ?? 0} in profit`} />
      <Metric label="Combined P&L" value={money(p.combined_pnl_usd)} amount={p.combined_pnl_usd} note={p.valuation_complete ? `${p.return_on_starting_capital_pct?.toFixed(2) ?? '—'}% on starting capital` : 'Awaiting valuation and ledger reconciliation'} />
      <Metric label="Recorded closed win rate" value={p.closed_win_rate == null ? '—' : `${p.closed_win_rate.toFixed(1)}%`} note={`${p.wins} wins / ${p.losses} losses / ${p.breakeven} flat`} />
    </div>
    {p.priced_positions < p.open_positions && <p className="executor-data-note">Valuation is incomplete. {p.open_positions-p.priced_positions} positions cannot be priced reliably ({money(p.unpriced_cost_usd)} recorded entry cost). Combined return and equity remain unavailable. Review the marked positions below.</p>}
    {p.flagged_closed_trades > 0 && <div className="executor-data-note"><strong>{p.flagged_closed_trades} closures contain verified price-unit errors.</strong><p>They account for {money(p.flagged_closed_pnl_usd)} of the recorded realized result. Excluding these records, the remaining closed-trade subtotal is {money(p.unflagged_realized_pnl_usd)}. This subset is not a corrected strategy return: the erroneous prices also triggered the exits.</p><details><summary>Review affected closures</summary>{p.closed_issues.map((t,i)=><p key={i}>{t.symbol} · {new Date(t.exit_time*1000).toLocaleDateString()} · recorded {money(t.pnl_usd)}<br/>{t.quality_issue}</p>)}</details></div>}
    ...
    <div className="executor-history-grid"><div><h3>Realized P&L over time</h3><p>Recorded closures, including flagged records. Open gains and losses are excluded.</p>
