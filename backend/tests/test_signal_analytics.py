...
def row(signal, outcome, met, conditions_met=9, regime="MARKUP"):
...
class IsWinTests(unittest.TestCase):
    ...
    def test_regime_scorecard_scores_shorts(self):
        rows = [row("LIGHT_SHORT", -5.0, {}), row("LIGHT_SHORT", -1.0, {}),
                row("LIGHT_SHORT", 2.0, {})]
...
class LongOnlyAverageTests(unittest.TestCase):
    ...
    ROWS = [
        row("LIGHT_LONG", 6.0, {"heat_ok": True}, conditions_met=12),
        row("STRONG_LONG", 4.0, {"heat_ok": True}, conditions_met=12),
        row("LIGHT_LONG", -2.0, {"heat_ok": False}, conditions_met=7),
    ...
    def test_confluence_buckets_use_long_entries_only(self):
        ...
        self.assertEqual(by_bucket["12+"]["avg_7d"], 5.0)
        low = by_bucket["<8"]
